MungCSV = {
	csv_to_json: function(file_info, user_info) {
		...
		MungCSV.read_file(file_url, file_info, user_info);
	},// END CSV_TO_JSON	
	
	read_file: function(file_url, file_info, user_info){
		...
		MungCSV.parse_file (file_data, file_info, user_info);
	...
	parse_file: function(file_data, file_info, user_info) {
		var parser = Meteor.npmRequire('csv-parse');
		
		parser(file_data, function(error, parsed_output){
			if (error) throw error;
			MungCSV.format_parsed_data(parsed_output, file_info, user_info);
		}); // END PARSER
	},// END PARSE_FILE
	
	format_parsed_data: function(parsed_output, file_info, user_info){
		...
		MungCSV.load_into_collections(file_props, file_rows, file_info, user_info);

	},// END format_parsed_data
	load_into_collections: function(file_props, file_rows, file_info, user_info) {
		// this where we'll actually load everything into the collections
	}
