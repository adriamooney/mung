MungCSV = {
	csv_to_json: function(file_info) {
		...
		var info = {
  		...
  		var ref_id = DataSets.insert(info);
	...
	format_parsed_data: function(file_info, parsed_output){
		...
		MungCSV.load_into_datasets(file_info, file_props, file_rows);
	}, 
	load_into_datasets: function(file_info, file_props, file_rows) {
		var ref_id = file_info.ref_id;

		//what i want to do is separate out the row data into a differnet data set
		// and make reference to the one being inserted now
		// this one will be the "summary" data data set
		// the other will be the detailed data data set
		...
		var info = {
  		...
  		DataSets.update(ref_id, {$set: info});
	}
};
