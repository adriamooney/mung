/*
// NOTE:
// 	IN ORDER TO REUSE CODE, YOU WILL SEE "!FILE_INFO.COLLECTION_REF_ID"
// 	THIS IS USED TO DETERMINE WHETHER THE USER IS UPLOADING A FRESH DATA SET
//  ** OR ** IF THEY'RE ADDING CLASSIFICATION DATA. 
*/
MungCSV = {
	csv_to_json: function(file_info) {
		...
		if (!file_info.collection_ref_id) {
			...
			var info = {
	  		...
	  		var ref_id = DataSetSummary.insert(info);
		...
		ServerSession.set('csv_to_json_running', 'done');
	},// END CSV_TO_JSON	
	...
	format_parsed_data: function(file_info, parsed_output){
		...
		MungCSV.load_into_dataset(file_info, file_props, file_rows);
	}, 
	load_into_dataset: function(file_info, file_props, file_rows) {

		// the following is all done now: 
		...
		if (!file_info.collection_ref_id) {
			var ref_id = file_info.ref_id;
	  		...
	  		var ref_id = file_info.ref_id;
			DataSetSummary.update(ref_id, {$set: info});
			// CONTROL FOR SIZE LIMITATIONS OF MONGO (HOPEFULLY!!)
			// THIS JUST CRAPS THE FUCK OUT
			/*
			if(file_info.size > 100000) {
				var info = {
						ref_id: ref_id,
				        properties: file_props,
				        row_data: file_rows[0]
			      };
		  		var conf_ref = DataSetData.insert(info);
		  		console.log(conf_ref);
		  		// now do the rest of the columns as an update
		  		for (var j = 1; j < file_props.length; j++) {
		  			var info = {
				        row_data: [file_rows[j]]
			      };
			      //console.log("------- FEATURE: " + j + " ------");
			      //console.log(info);
			      DataSetSummary.update({ref_id: ref_id}, {$addToSet: info});
		  		}
		  	} else { // it's a "small" dataset and can just be inserted
		  	*/
		  		var info = {
					ref_id: ref_id,
			        properties: file_props,
  		...
  		} else if (file_info.collection_ref_id) {
  			var info = {
  				classifiers: file_rows[1]
  			};
  			var ref_id = file_info.collection_ref_id;
  			var conf_ref = DataSetData.update({ref_id: ref_id}, {$set: info});
  			console.log(conf_ref);

  			// CATALOG THE INCLUSION OF CLASSIFICATION DATA INTO SUMMARY 
  			// (SO NOT PROPMTED IN FUTURE)
  			var info = {
  				has_classification: 'TRUE'
  			}
  			var conf_ref = DataSetSummary.update(ref_id, {$set: info});
  			console.log(conf_ref);
  		}
	}
};
