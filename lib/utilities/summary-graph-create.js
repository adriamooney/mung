	...
	struct_graph_data: function(x_val, prop_data, property) {
		...
		*/
		...
		for (var j = 0; j<prop_data.length; j++) {
			// START with line graphs
			if (isNaN(prop_data[j]) == false) {
					values[j] = {
					x: x_val[j],
					y: prop_data[j]
				};
				...
				{
				// check for match on prop_data[j] = container[i]
				if (container.length > 0) {
					for (var i = 0; i < container.length; i++) {
							if (prop_data[j] = container[prop_data[j]]) {
								container[prop_data[j]]++;
								console.log(container[prop_data[j]]);
							}
						}// no "strings" in container (yet)
					}
				else {
					container[prop_data[j]] = prop_data[j];
