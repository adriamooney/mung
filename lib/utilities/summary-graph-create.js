MungSummaryGraph = {
	...
	struct_bar_data: function(x_val, prop_data, property) {
		...
		*/
		...
		for (var j = 0; j<prop_data.length; j++) {
			// see if there are any previous 'lable's we need to be aware of
			if (values.length == 0) {
				var new_field = {
					label: prop_data[j],
					value: 1
				...
				for (var i = 0; i < values.length; i++) {
						if (values[i].label == prop_data[j]) {
							values[i].value++;
						} // THIS ELSE BREAKS IT FOR SOME REASON!!
						/* else { // if there's no match after looping
						// need to add the new field to the array
						var new_field = {
							label: prop_data[j],
							value: 1
							};
