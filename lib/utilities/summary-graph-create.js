MungSummaryGraph = {
	init: function(summary_graph_list){
		...
		var dataset_summary_id = summary_graph_list.dataset_summary_id;
		this.dataset_summary_data = DataSetSummary.findOne({_id: dataset_summary_id});
		console.log(this.dataset_summary_data);
		...
		for (var i = 0; i < status_array.length; i++) {
			if(status_array[i] == 1) {
				...
				var data_values = this.dataset_summary_data.file_data.row_data[i];
				var property = this.dataset_summary_data.file_data.properties[i];
				
				// need to namespace out row_mean (for now) 
				// because if it doesn't exist, then i need it to
				this.dataset_summary_data.file_data.row_means = this.dataset_summary_data.file_data.row_means || [];

				// only get basic stats if not already calculated
				/*	if (this.dataset_summary_data.file_data.row_means[i]) { 
