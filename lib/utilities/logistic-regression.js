MungLogisticRegression = {
	init: function(data_set_id, num_iterations){
		...
		*/ 
		...
		// remember: sigmoid returns a probablity: p(y|x; theta);
			...
			theta = MungLogisticRegression.gradient_descent(theta, grad[j], alpha);
			if (cost[j-1]) {
				 if (cost[j-1] > cost[j]) {
					alpha = alpha * .6;
				} else if (cost[j-1] < cost[j]) {
					alpha = alpha * 1.7;
				} else if (isNaN(cost[j])){
					alpha = alpha * .3;
				}
			}
		}// END ITERATION LOOP
			...
			// TEST STUFF
			var test_ex = [1, 45, 85];
			var test_hypoth = MungLogisticRegression.calc_hypoth(test_ex, theta);
			var test_sig = MungLogisticRegression.calc_sigmoid(test_hypoth);
	...
	gradient_descent: function(current_theta, current_gradient, alpha) {
			/*
			// error = hypothesis - y;
			// grad = (X' * error) / m;
