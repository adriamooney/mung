MungSummaryStats= {
	init: function(data_summary_id){
		...
		if(!data_set_summary.feature_summary){
	...
	calculate_mean: function(data_summary_id) {
		...
		for (i = 0; i < data.row_data.length; i++) {
			...
			if (isNaN(value_total) == false) {
				...
				// get the other stuff we need here 
				// BECAUSE mongo was being a thorn in the ass
				// and this is easier (though not ideal)
				// 	GET VARIANCE AND SD (w/ mean)
				var basic_stats = this.calculate_sd(current_column, mean);
				//console.log(mean);
				var update_data = {
						feature_summary: {
					...
					// insert mean, variance, sd into DataSetSummary
					DataSetSummary.update({_id: data_summary_id}, {$push: update_data});
			} // END DATA.ROW_DATA LOOP
		}
	},
	calculate_variance: function(data, mean){
	...
	calculate_sd: function(data, mean) {
		/* 
		...
		*/
		...
		var variance = this.calculate_variance(data, mean);	
		var standard_deviation = Math.sqrt(variance);
		//console.log(standard_deviation);
		var basic_stats = {variance: variance, sd: standard_deviation};
			/* var sum_data = {
				 		row_sum : { 
							variance: variance,
			...
			*/
			return basic_stats;
			//DataSetSummary.update(data_sum, {$addToSet: sum_data});
	},
